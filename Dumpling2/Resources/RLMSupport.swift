...
//import Realm
...
#else
    extension RLMRealm {
        @nonobjc public class func schemaVersionAtURL(_ url: URL, encryptionKey key: Data? = nil,
                                                      error: NSErrorPointer) -> UInt64 {
            return __schemaVersion(at: url, encryptionKey: key, error: error)
        }
    }
    ...
    extension RLMObject {
        ...
        public class func objectsInRealm(_ realm: RLMRealm,
                                         _ predicateFormat: String,
                                         _ args: CVarArg...) -> RLMResults {
            return objects(in: realm,
                                  with: NSPredicate(format: predicateFormat, arguments: getVaList(args)))
        }
    }

    public final class RLMGenerator: IteratorProtocol {
    ...
    extension RLMArray: Sequence {
        // Support Sequence-style enumeration
        public func makeIterator() -> RLMGenerator {
